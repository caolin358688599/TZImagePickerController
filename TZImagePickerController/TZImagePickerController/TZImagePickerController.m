...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc mainColor:(UIColor *)mainColor listImageVideo:(BOOL)listImageVideo type:(NSInteger)type {
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            [self pushPhotoPickerVc:listImageVideo type:type];
        }
    }
    return self;
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate mainColor:(UIColor *)mainColor listImageVideo:(BOOL)listImageVideo{
...
//    }
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        TZPhotoPickerController *photoPickerVc = [[TZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.isListImageVideo = listImageVideo;
        if (type == 1) {
            photoPickerVc.isPhoto = YES;
        } else {
            photoPickerVc.isPhoto = false;
        }
        photoPickerVc.columnNumber = self.columnNumber;
        if (_mainColor) {
            photoPickerVc.mainColor = _mainColor;
